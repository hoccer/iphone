...
- (id) initWithScrollView:(UIScrollView *)scrollView delegate:(id<CustomPullToRefreshDelegate>)delegate
{
    ...
    if (self) {
        ...
        for (int i=1; i<5; i++) {
            [animationImages addObject:[UIImage imageNamed:[NSString stringWithFormat:@"receive-step-%d.png",i]]];
        }
//            [animationImages addObject:[UIImage imageNamed:[NSString stringWithFormat:@"loading-%d.png",i]]];
        ...
        _rainbowBot.frame = CGRectMake(0, -200, _scrollView.frame.size.width, 200);
        _rainbowBot.animationImages = animationImages;
        _rainbowBot.animationDuration = 5;
...
//    NSLog(@"%@",NSStringFromCGSize(_scrollView.frame.size));
